...
def binding(word,word_eol,userdata):
	if hexchat.strip(word[0]) in OMNOMIRCNICK:
		msg = word[1]
		res = re.match(r'^(\x03[0-9]{1,2}\([OC]\)\x0F|\(#\))<([^>]+)> (.*)',msg)
		textEvent = ''
		args = []
		while True:
			if res: # normal msg
				...
				args = [res.group(3)]
				break
			res = re.match(r'^(\x03[0-9]{1,2}\([OC]\)\x0F|\(#\))\x036\* ([^ ]+) (.*)',msg)
			if res: # action
				if doHighlight(res.group(3)):
				...
				args = [res.group(3)]
				break
			res = re.match(r'^(\x03[0-9]{1,2}\([OC]\)\x0F|\(#\))\x032\* ([^ ]+) has left ([^ ]*) \((.*)\)',msg)
			if res: # part
				if res.group(4)!='':
			...
			if res: # quit
				...
				args = [res.group(3)]
				break
			res = re.match(r'^(\x03[0-9]{1,2}\([OC]\)\x0F|\(#\))\x033\* ([^ ]+) set ([^ ]*) mode (.*)',msg)
			if res: # mode
				textEvent = 'Channel Mode Generic'
				args = ['',res.group(4),res.group(3)]
				break
			res = re.match(r'^(\x03[0-9]{1,2}\([OC]\)\x0F|\(#\))\x034\* ([^ ]+) has kicked ([^ ]*) from ([^ ]*) \((.*)\)',msg)
			if res: # kick
				textEvent = 'Kick'
				args = [res.group(3)+'@OmnomIRC'+res.group(1),res.group(4),res.group(5)]
				break
			res = re.match(r'^(\x03[0-9]{1,2}\([OC]\)\x0F|\(#\))\x033\* ([^ ]+) has changed the topic to (.*)',msg)
			if res: # topic
